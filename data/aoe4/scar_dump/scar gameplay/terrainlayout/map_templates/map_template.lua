...
--These tables hold necessary river info. The coordinates that are put in these tables are what tells the mapgen systems where to generate the river and crossings.
riverResult = {}
fordResults = {}
woodBridgeResults = {}
stoneBridgeResults = {}

--You can manually draw lines of terrain and set them to be saved in the riverResult.

--In this case, we are randonly choosing a starting and ending row for a river that will run horizontally across the map.
riverStartRow = math.ceil(worldGetRandom() * #terrainLayoutResult)
riverStartCol = 1
riverEndRow = math.ceil(worldGetRandom() * #terrainLayoutResult)
riverEndCol = #terrainLayoutResult

--We are drawing a straight line of points from the starting to the ending coordinates, to save the river path
...
--assign all river squares to be river terrain type
...
--make a stone bridge in the middle of the river
--to do this, we insert a coordinate of a point on the river into the stoneBridgeResult
stoneBridgeTable = {}
riverHalfway = math.ceil(#riverResult / 2)
table.insert(stoneBridgeTable, riverResult[1][riverHalfway])
