...
originTerrain = tt_savannah_player_start
spawnTerrain = tt_plains_clearing

swampTerrain = tt_savannah_swamp
swampForageTerrain = tt_savannah_swamp_forage
waterTerrain = tt_savannah_water
...
-- Creating a small lake in the middle of the map.
local terrainType = swampTerrain
...
if (isZeroPlayerGame ~= true) then
	...
	local avoidanceTable = {baseTerrain, swampTerrain, waterTerrain}
	...
	local avoidanceTerrain = {baseTerrain, swampTerrain, waterTerrain}
...
local acceptanceTerrain = baseTerrain
local avoidanceTerrain = {spawnTerrain, swampTerrain, waterTerrain}
...
local terrainTable = tt_savannah_gold
GenerateTerrainsFromTable(tableName, terrainTable)

-- Defining neutral stone clumps for the base zone.
local acceptanceTerrain = baseTerrain
local avoidanceTerrain = {spawnTerrain, swampTerrain, waterTerrain, tt_savannah_gold}
local locateNumber = 2
local distributionType = "player"
local separationDistance = 12
local avoidanceDistance = 1
...
local terrainTable = tt_savannah_stone
GenerateTerrainsFromTable(tableName, terrainTable)
...
local terrainTable = tt_savannah_chonker
GenerateTerrainsFromTable(tableName, terrainTable)
...
local terrainTable = tt_savannah_herdable
GenerateTerrainsFromTable(tableName, terrainTable)
...
local terrainTable = tt_savannah_ungulate
GenerateTerrainsFromTable(tableName, terrainTable)

-- Defining swamp clumps for varied map elevation.
local acceptanceTerrain = baseTerrain
local avoidanceTerrain = {spawnTerrain, swampTerrain, waterTerrain, tradeTerrain}
...
local edgeAvoidance = 1
local theTable = baseZone
local swampTable = CreateClumpsFromTable(theTable, acceptanceTerrain, avoidanceTerrain, locateNumber, distributionType, separationDistance, avoidanceDistance, minimumDistance, edgeAvoidance, forceEquality)

-- Generating the actual swamp on the clumps.
local tableName = swampTable
local terrainTable = {
	{swampTerrain, 5},
	{swampForageTerrain, 1}
}
GenerateTerrainsFromTable(tableName, terrainTable)

-- Generating a holy site in the middle of the swamp.
terrainLayoutResult[mapCentre][mapCentre].terrainType = tt_savannah_island
