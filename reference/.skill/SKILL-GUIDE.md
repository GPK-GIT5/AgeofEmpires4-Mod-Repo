...
- AoE4 workspace with `data/` folder (buildings, units, technologies, civilizations)
...
The cache is automatically marked as stale if source files (data/buildings/all.json, etc.) are newer than cached timestamps:
...
**Solution:**
1. Check source file timestamps: `Get-Item data/*/all.json | Select FullName, LastWriteTime`
