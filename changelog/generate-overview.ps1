...
# Script lives in changelog/ directory; $PSScriptRoot is the scope parent
$scopePath = Join-Path $PSScriptRoot $Scope
$indexFile = Join-Path $scopePath "INDEX.jsonl"

# Determine current month file (YYYY-MM.md)
...
if (-not (Test-Path $mdFile)) {
    ...
    Write-Host "✓ Created $currentMonth.md" -ForegroundColor Yellow
}
...
if ($entries.Count -eq 0) {
    ...
    Write-Host ".\validate-entry.ps1 -Entry '{...}'" -ForegroundColor Yellow
...
foreach ($group in $grouped) {
    ...
    # Generate anchor link (simple format: #YYYY-MM-DD)
    $anchor = "#$date"
    
    $tableRows += "| $date | $countDisplay | $keyAreas | [→]($anchor) |"
}
