...
- Run `validate-entry.ps1` before appending changelog entries
...
```powershell
# 1. Validate entry
.\validate-entry.ps1 -Entry '{"timestamp":"...","file":"...","status":"modified",...}'

# 2. Append to scope index
Add-Content changelog/<scope>/INDEX.jsonl '<json>'

# 3. Regenerate overview
.\generate-overview.ps1 -Scope <scope>
```

3-scope system: `mods` | `system` | `workspace`. 9 required fields per entry.
